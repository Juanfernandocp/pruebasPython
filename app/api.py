...
@api_application.route("/calc/check_types/<op_1>/<op_2>", methods=["GET"])
def check_types(op_1, op_2):
    try:
        num_1, num_2 = util.convert_to_number(op_1), util.convert_to_number(op_2)
        return ("{}".format(CALCULATOR.check_types(num_1, num_2)), http.client.OK, HEADERS)
    except TypeError as e:
        return (str(e), http.client.BAD_REQUEST, HEADERS)
...
def logBase10(op_1):
    try:
        num_1= util.convert_to_number(op_1)
        return ("{}".format(CALCULATOR.logBase10(num_1)), http.client.OK, HEADERS)
    except TypeError as e:
        return (str(e), http.client.BAD_REQUEST, HEADERS)
